...
export async function rewriteFinding(finding: Finding, deps: RewriteDeps): Promise<RewriteResult> {
  ...
  while (attempt < 3) {
    const prompt = fillPrompt(template, {
      ...
      comment_type: finding.review?.kind ?? 'warning',
      auto_fix_allowed: finding.review?.autoFixable ? 'yes' : 'no',
      manual_review_required: finding.review?.requiresManualReview ? 'yes' : 'no',
